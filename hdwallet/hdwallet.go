...
// NewTransactor is a utility method to easily create a transaction signer
func (w *Wallet) NewTransactorWithChainID(account accounts.Account, chainID *big.Int) (*bind.TransactOpts, error) {
	signer := types.LatestSignerForChainID(chainID)
	if chainID == nil {
	...
	return &bind.TransactOpts{
		...
		Signer: func(address common.Address, tx *types.Transaction) (*types.Transaction, error) {
			...
			signature, err := w.SignHash(account, signer.Hash(tx).Bytes())
			if err != nil {
				return nil, err
			}
