...
func Compress(inputFilePath, outputFilePath string) (err error) {
	...
	undoDir, err := mkdirAll(filepath.Dir(outputFilePath), 0o755)
	if err != nil {
		return err
	}
	defer func() {
...
func Extract(inputFilePath, outputFilePath string) (err error) {
	...
	undoDir, err := mkdirAll(outputFilePath, 0o755)
	if err != nil {
		return err
	}
	defer func() {
...
func extract(filePath string, directory string) error {
	...
	for {
		...
		if fileInfo.IsDir() {
			err = os.Chmod(dir, fileInfo.Mode())
			if err != nil {
				return err
			}
			err = os.Chtimes(dir, fileInfo.ModTime(), fileInfo.ModTime())
			if err != nil {
				return err
			}
			continue
		...
		file, err := os.OpenFile(filename, os.O_CREATE|os.O_TRUNC|os.O_WRONLY, fileInfo.Mode())
		if err != nil {
			return err
		}
		...
		err = os.Chtimes(filename, fileInfo.ModTime(), fileInfo.ModTime())
		if err != nil {
			return err
		}
	}
